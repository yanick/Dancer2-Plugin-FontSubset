...
has fonts_dir => (
    is => 'ro',
    lazy => 1,
    from_config => sub { 'fonts' },
);

has font_base_url => (
    is => 'ro',
    lazy => 1,
    from_config => sub { '/font' },
);
...
has cache => (
    ...
    lazy => 1,
    default => sub {
        $_[0]->find_plugin('Dancer2::Plugin::Cache::CHI');
    },
);
...
sub BUILD {
    ...
    $self->app->add_route(
        method =>  'get',
        regexp => $self->font_base_url . '/subset.js',
        code => $self->_subset_route,
    );

    $self->app->add_route(
        method => 'get',
        regexp => $self->font_base_url . '/:fontname',
        code => $self->_font_route,
    );
...
sub _subset_route {
    ...
    return sub { sprintf <<'END_JS', $self->font_base_url };
$(function(){
...
sub _font_route {
    ...
    return sub {
        ...
        my $fontname = $app->request->params->{fontname};

        $fontname =~ s#\.\.##g; # just to be safe

        my $path = $plugin->font_path( $fontname );

#        $app->send_error( 'font not found', 404 ) unless -f $path;

        # no text? No job to do
        my $text = $app->request->params->{t} 
            // return $app->send_file( $path );

        my @chars = map { ord } sort { $a cmp $b } uniq split //, $text; 

        $path = join '/', $plugin->app->config->{public_dir}, $path;

        my $output = $plugin->generate_subfont( $path, @chars );

        return $app->send_file( \$output, content_type => 'application/x-ttf' );
    }
};
...
sub generate_subfont {
    my( $self, @args ) = @_;

    return $self->_generate_subfont(@args) unless $self->use_cache;

    return $self->cache->cache->compute(
        'font-' . $args[0] . '-' . join( '', @args[1..$#args] ), sub {
            $self->_generate_subfont(@args);
        }
    );
}
